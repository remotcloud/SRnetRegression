...
def _parallel_evolve(i,n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    def _tournament(j):
        ...
        fitness = [parents[p][0][j].fitness_ for p in contenders]
    ...
    for i in range(n_programs):
        ...
        for j in range(n_features):
            ...
            if parents[j] is None:
                ...
                parent, parent_index = _tournament(j)

                if method < method_probs[0]:
                    # crossover
                    donor, donor_index = _tournament(j)
                    program, removed, remains = parent[0][j].crossover(donor[0][j].program,
                                                                 random_state)          #donor 和parent都是一个n元组
                    ...
                    # subtree_mutation
                    program, removed, _ = parent[0][j].subtree_mutation(random_state)
                    genome = {'method': 'Subtree Mutation',
                              'parent_idx': parent_index,
                              'parent_nodes': removed}
                elif method < method_probs[2]:
                    # hoist_mutation
                    program, removed = parent[0][j].hoist_mutation(random_state)
                    genome = {'method': 'Hoist Mutation',
                              'parent_idx': parent_index,
                              'parent_nodes': removed}
                elif method < method_probs[3]:
                    # point_mutation
                    program, mutated = parent[0][j].point_mutation(random_state)
                    genome = {'method': 'Point Mutation',
                              'parent_idx': parent_index,
                              'parent_nodes': mutated}
                else:
                    # reproduction
                    program = parent[0][j].reproduce()
        ...
        program_n.append(program_set) #记录所有的progame
        #使用线性回归求系数w

        y_fitness = []
        y_weight = []  #权值
        y_intercept = []  #截距
        program_x = program_n[0]   #调用所有的program
        for m in range(y_features):
            x_trainl = []
            y_pre = 0
            for k in range(n_features):
                x_n = X[:, k][:, np.newaxis]  # 每个x的公式对应的x
                x_turn = program_x[k].execute(x_n)
                #print(x_turn.shape)
                if len(x_turn.shape)==1:
                    x_turn = x_turn[:, np.newaxis]
                #print(x_turn.shape)
                x_trainl.append(x_turn)
            x_train = x_trainl[0]
            for k in range(n_features):
                if k>0:
                    #print(x_trainl[k].shape)
                    x_train = np.concatenate((x_train,x_trainl[k]),axis=1)

            regr = linear_model.Ridge(alpha=.5)
            y_true = y[:, m][:, np.newaxis]  # 每个y的真实值

            regr.fit(x_train, y_true)
            #if the coefficient of the model smaller than a threshold, make it euqal to 0
            regr.coef_[np.where(regr.coef_ < 0.01)] = 0
            y_pre = regr.predict(x_train)
            y_weight.append(regr.coef_)
            y_intercept.append(regr.intercept_)
            raw_fitness_ = metric(y_true, y_pre, sample_weight)
            y_fitness.append(raw_fitness_)


        y_fitness_array = np.array(y_fitness)
        fitness_true = np.average(y_fitness_array)
        for k in range(n_features):                                 #n——feature个x的种群赋值fitness
            program_n[0][k].raw_fitness_ = fitness_true

        program_n.append(y_weight)
        program_n.append(y_intercept)

        if max_samples < n_samples:
            # Calculate OOB fitness
            program.oob_fitness_ = program.raw_fitness(X, y, oob_sample_weight)

        programs.append(program_n)
...
class BaseSymbolic(six.with_metaclass(ABCMeta, BaseEstimator)):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for gen in range(prior_generations, self.generations):
            ...
            parents = []
            if gen == 0:
                for p in range(self.n_features_):
                    parents.append(None)
            else:
            ...
            population = Parallel(n_jobs=n_jobs,
                ...
                delayed(_parallel_evolve)(i,n_programs[i],
                                          parents,
                                          X,
                                          y,
                                          sample_weight,
            ...
            # all fitness is same
            fitness = [program[0][0].raw_fitness_ for program in population]   #每一个都是一个n元组

            length = [program[0][0].length_ for program in population]
            ...
            for program in population:
                for j in range(self.n_features_):
                    program[0][j].fitness_ = program[0][j].fitness(parsimony_coefficient)
                        ...
                        for j in range(self.n_features_):
                            if program is not None:
                                if program[0] is not None:
                                    if program[0][j] is not None:
                                        for idx in program[0][j].parents:
                                            if 'idx' in idx:
                                                indices.append(program[0][j].parents[idx])
                    indices = set(indices)
            ...
            # Record the best fitness of the gen
            if self._metric.greater_is_better:
                self._fittrend.append(fitness[np.argmax(fitness)])
            else:
                self._fittrend.append(fitness[np.argmin(fitness)])
            #求出前several个最好的个体
            several = 1
            if self._metric.greater_is_better:
                fitness_min_or_max = np.argpartition(fitness,-several)[-several:]
            else:
                fitness_min_or_max = np.argpartition(fitness, several)[:several]

            for i in range(several):
                self._best_pro.append(population[fitness_min_or_max[i]])
                self._fitness.append(population[fitness_min_or_max[i]][0][0].raw_fitness_)
            ...
            self.run_details_['best_length'].append(best_program[0][0].length_)
            self.run_details_['best_fitness'].append(best_program[0][0].raw_fitness_)
            oob_fitness = np.nan
            if self.max_samples < 1.0:
        ...
        #所有代数中前several好的个体
        several = 1
        if len (self._fitness) > several:
            if self._metric.greater_is_better:
                fitness_min_or_max = np.argpartition(self._fitness, -several)[-several:]
            else:
                fitness_min_or_max = np.argpartition(self._fitness, several)[:several]
            self._fitness  = []

            for i in range(several):
                self.best_pro_several.append(self._best_pro[fitness_min_or_max[i]])
                self.fitset.append(self._best_pro[fitness_min_or_max[i]][0][0].raw_fitness_)
        else:
            for i in range(several):
                self.best_pro_several.append(self._best_pro[i])
                self.fitset.append(self._best_pro[i][0][0].raw_fitness_)
