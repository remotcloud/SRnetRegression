...
#sd 记录结果
'''txtIndividul = 'log/'+str(funName)+'/run.txt'
file_handle = open(txtIndividul, mode='a',encoding='utf-8')
txtRun = 'The '+str(i+1)+' run result:\n'
file_handle.write(txtRun)
for best in best_individualSet:
    file_handle.writelines(['适应度为:',str(best[0][0].raw_fitness_),'\n'])
    for j in best[0]:
        j = trans_expression(str(j),True)
        file_handle.writelines([' fomula：',str(j),'\n'])
'''
